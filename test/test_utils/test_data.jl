...
function test_2_zone_portfolio()
    ...
    # Data added in MW
    ts_demand_2030 = SingleTimeSeries("ops_peak_load", TimeArray(tstamp_2030_ops, ts_load_2030))
    ts_demand_2035 = SingleTimeSeries("ops_peak_load", TimeArray(tstamp_2035_ops, ts_load_2035))
    ...
    colocated_unit = PSIP.ColocatedSupplyStorageTechnology{RenewableDispatch}(;
        name="colocated_test",
        available=true,
        id=10,
        power_systems_type="EnergyReservoirStorage",
    ...
    line = AggregateTransportTechnology{ACBranch}(
        ...
        start_region=z1,
        end_region=z2,
        capacity_limits=(min=0.0, max=9000),
        line_loss=0.05,
    ...
    PSIP.add_technology!(p_5bus, colocated_unit)
    ...
    existing_ch_thermal = ExistingCapacity(existing_technologies=cheap_th_names)
    existing_exp_thermal = ExistingCapacity(existing_technologies=exp_th_names)
    existing_renewable = ExistingCapacity(existing_technologies=[])

    PSIP.add_supplemental_attribute!(p_5bus, t_th_exp, existing_exp_thermal)
    PSIP.add_supplemental_attribute!(p_5bus, t_th, existing_ch_thermal)
    PSIP.add_supplemental_attribute!(p_5bus, t_re, existing_renewable)
